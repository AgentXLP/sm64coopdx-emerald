...
void network_send_area(struct NetworkPlayer *toNp) {
    ...
    packet_ordered_begin();
    {
        ...
        // write respawners
        for (struct SyncObject *so = sync_object_get_first(); so != NULL; so = sync_object_get_next()) {
            if (so == NULL || so->o == NULL || so->o->behavior != smlua_override_behavior(bhvRespawner)) {
                continue;
            }
        ...
        for (struct SyncObject *so = sync_object_get_first(); so != NULL; so = sync_object_get_next()) {
            if (so == NULL || so->o == NULL || so->o->oSyncID != so->id) {
                continue;
            }
            if (so->o->behavior == smlua_override_behavior(bhvRespawner)) {
                continue;
            }
        ...
        for (struct SyncObject *so = sync_object_get_first(); so != NULL; so = sync_object_get_next()) {
            ...
            if (so->lastReliablePacketIsStale) {
                continue;
            }
    ...
    for (s32 i = 0; i < respawnerCount; i++) {
        ...
        if (so == NULL) {
            ...
            LOG_DEBUG("rx area debug: Sync Object DEBUG:\n\n \
                      ...
                      posZ,
                      behParams,
                      respawnerModelToRespawn,
                      respawnerMinSpawnDist,
                      behaviorToRespawn,
                      syncId);
            continue;
        }
