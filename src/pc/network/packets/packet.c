...
static void increase_comp_buffer(u32 compressedLen) {
    if (compressedLen <= sCompBufferLen && sCompBuffer) {
        return;
    }
...
void packet_process(struct Packet *p) {
    ...
    if (p->levelAreaMustMatch) {
        ...
        bool levelAreaMismatch =
            (p->courseNum != gCurrCourseNum || p->actNum != gCurrActStarNum || p->levelNum != gCurrLevelNum || p->areaIndex != gCurrAreaIndex);
        // drop packet
        ...
        bool levelMismatch =
            (p->courseNum != gCurrCourseNum || p->actNum != gCurrActStarNum || p->levelNum != gCurrLevelNum);
        // drop packet
        if (levelMismatch) {
            if (gNetworkType != NT_SERVER) {
                LOG_INFO("dropping level mismatch packet %d", p->packetType);
