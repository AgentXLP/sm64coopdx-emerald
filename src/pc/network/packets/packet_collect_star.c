...
void network_receive_collect_star(struct Packet *p) {
    ...
    if (gSaveFileUsingBackupSlot != 0) {
        gSaveFileUsingBackupSlot = 1;
    }
    ...
    bool levelAreaMismatch = ((np == NULL) || np->currCourseNum != gCurrCourseNum || np->currActNum != gCurrActStarNum || np->currLevelNum != gCurrLevelNum || np->currAreaIndex != gCurrAreaIndex);
