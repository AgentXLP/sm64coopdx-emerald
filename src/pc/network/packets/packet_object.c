...
static struct SyncObject *packet_read_object_header(struct Packet *p, u8 *fromLocalIndex) {
    ...
    *fromLocalIndex = (np != NULL) ? np->localIndex : p->localIndex;
    ...
    if (o->behavior != behavior && o->behavior != lBehavior && !allowable_behavior_change(so, behavior)) {
        ...
        LOG_ERROR(
            "during read behavior mismatch for %d: %04X (%s) vs %04X (%s)", syncId, objBehaviorId, get_behavior_name_from_id(objBehaviorId), inBehaviorId, get_behavior_name_from_id(inBehaviorId));
        return NULL;
    }
...
void network_send_object(struct Object *o) {
    ...
    if (o->behavior != so->behavior && !allowable_behavior_change(so, so->behavior)) {
        ...
        LOG_ERROR(
            "during send behavior mismatch for %d: %04X (%s) vs %04X (%s)", o->oSyncID, objBehaviorId, get_behavior_name_from_id(objBehaviorId), soBehaviorId, get_behavior_name_from_id(soBehaviorId));
        sync_object_forget(so->id);
        return;
    }
...
void network_send_object_reliability(struct Object *o, bool reliable) {
    ...
    if (o->behavior != so->behavior && !allowable_behavior_change(so, so->behavior)) {
        ...
        enum BehaviorId soBehaviorId = get_id_from_behavior(so->behavior);
        LOG_ERROR(
            "during send reliability behavior mismatch for %d: %04X (%s) vs %04X (%s)", syncId, objBehaviorId, get_behavior_name_from_id(objBehaviorId), soBehaviorId, get_behavior_name_from_id(soBehaviorId));
        sync_object_forget(so->id);
        return;
    }
...
void network_update_objects(void) {
    ...
    for (struct SyncObject *so = sync_object_get_first(); so != NULL; so = sync_object_get_next()) {
        ...
        if (so->maxSyncDistance != SYNC_DISTANCE_INFINITE && dist > so->maxSyncDistance) {
            continue;
        }
        float updateRate = dist / 1000.0f;
        if (gMarioStates[0].heldObj == so->o) {
            updateRate = 0.33f;
        }

        // set max and min update rate
        if (so->maxUpdateRate > 0 && updateRate < so->maxUpdateRate) {
            updateRate = so->maxUpdateRate;
        }
        if (updateRate < so->minUpdateRate) {
